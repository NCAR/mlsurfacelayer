...
#                      "potential_temperature:30_m:K",
    ...
    #
    derived_data["wave_dir_interp:0_m:degrees"] = raw_data["Value_wave_dir:Buoy:degree"]
    derived_data["wave_dir_nearest_interp:0_m:degrees"] = raw_data["Value_wave_dir_next_interp:Buoy:degree"]
    ...
    for height in [40, 60, 80]:
        ...
        # Wind Speed 
        #
        derived_data[f"wind_speed:{height:d}_m:m_s-1"] = raw_data[f"Value_Wind_Speed:{height:d}_m:m/s"]

        #
        # Wind Direction
        #
        derived_data[f"wind_direction:{height:d}_m:degrees"] = raw_data[f"Value_Wind_Direction:{height:d}_m:degree"]
    ...
    #    
    # Temperature at 40,50,70,100m.  For 60 and 80m raw data interpolate linearly using measured values at 50 and 70m. 
    #
    for height in [40,50,70,100]: 
        derived_data[f"temperature:{height:d}_m:K"] =  celsius_to_kelvin(raw_data[f"Value_Air_Temperature:{height:d}_m:c"])
    
    derived_data["temperature:60_m:K"] = celsius_to_kelvin( (raw_data['Value_Air_Temperature:50_m:c'] + raw_data['Value_Air_Temperature:70_m:c'] )/2 )
    derived_data["temperature:80_m:K"] = celsius_to_kelvin(raw_data['Value_Air_Temperature:70_m:c'] + (raw_data['Value_Air_Temperature:70_m:c'] - raw_data['Value_Air_Temperature:50_m:c'] )/2 )

    #
    # flux components (correcting for incorrect units of input data)
    #
    
    #derived_data["u_w:40_m:m2_s-2"] = raw_data["u_w:40_m:m2_s-2"]
    derived_data["u_w:60_m:m2_s-2"] = raw_data["u_w:60_m:m/s"]
    derived_data["u_w:80_m:m2_s-2"] = raw_data["u_w:80_m:m/s"]
    #derived_data["v_w:40_m:m2_s-2"] = raw_data["v_w:40_m:m2_s-2"]
    derived_data["v_w:60_m:m2_s-2"] = raw_data["v_w:60_m:m/s"]
    derived_data["v_w:80_m:m2_s-2"] = raw_data["v_w:80_m:m/s"]


    #    
    # Relative humidity at 30,40,50,60,80m.  Interpolate linearly using measured values at 30 and 90m. 
    #
    derived_data["relative_humidity:30_m:%"]=  raw_data['hygro_mean:30_m:percent']
    derived_data["relative_humidity:40_m:%"] = raw_data['hygro_mean:30_m:percent'] + (raw_data['hygro_mean:90_m:percent'] - raw_data['hygro_mean:30_m:percent'])/6 
    derived_data["relative_humidity:50_m:%"] = raw_data['hygro_mean:30_m:percent'] + (raw_data['hygro_mean:90_m:percent'] - raw_data['hygro_mean:30_m:percent'])/3
    derived_data["relative_humidity:60_m:%"] = raw_data['hygro_mean:30_m:percent'] + (raw_data['hygro_mean:90_m:percent'] - raw_data['hygro_mean:30_m:percent'])/2
    derived_data["relative_humidity:80_m:%"] = raw_data['hygro_mean:90_m:percent'] - (raw_data['hygro_mean:90_m:percent'] - raw_data['hygro_mean:30_m:percent'])/6
    ...
    #
    heights = [40,60,80]
    derived_data[f"friction_velocity:{height:d}_m:m_s-1"] = raw_data[f"ustar:{height:d}_m:u'w'"]
...
def load_derived_data_random_test_train(filename, dropna=True, filter_counter_gradient=False): 
    ...
    # For testing cubist
    #
    train.to_csv("/d1/FINO1/cubist_fino_2006_2010_train.csv", na_rep = '?')
    test = pd.DataFrame()
    test = all_data.loc[all_data.index.difference(data["test"].index) ]
    test.to_csv("/d1/FINO1/cubist_fino_2006_2010_test.csv", na_rep = '?')
    return data

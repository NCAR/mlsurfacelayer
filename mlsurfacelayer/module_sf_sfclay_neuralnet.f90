module module_sf_sfclay_neuralnet
    use module_neuralnet
    implicit none

    type neuralnet_sfclay
    ...
    type(neuralnet_sfclay), save :: nn_sfc

contains

    subroutine sfclay_neuralnet(ims, ime, jms, jme, kms, kme, &
...
!        real, intent(in) :: r_d, cp, xlv, karman
        ...
        enddo
    end subroutine sfclay_neuralnet
    ...
    subroutine sfclay_neuralnet_2d(its, ite, kts, kstop, &
        ...
        do i = its, ite
            ...
            ust_inputs = (/ wind_speed(kts), wind_speed(kstop), pot_temp_skin_diff_1, qv_skin_diff_1, br(i), zenith /)
            tstar_inputs = (/ qv_skin_diff_1, qv_skin_diff_2, pot_temp_skin_diff_1, &
                pot_temp_skin_diff_2, br(i), wspd(kts), wind_speed(kstop), zenith /)
            qstar_inputs = (/ qv_skin_diff_1, qv_skin_diff_2, pot_temp_skin_diff_1, &
                pot_temp_skin_diff_2, br(i), wspd(kts), wind_speed(kstop), zenith, mavail(i) /)
            ! Run random forests to get ust, mol, and qstar
            ust(i) = random_forest_predict(real(ust_inputs, 8), nn_sfc%friction_velocity)
            mol(i) = random_forest_predict(real(tstar_inputs, 8), nn_sfc%temperature_scale)
            qstar(i) = random_forest_predict(real(qstar_inputs, 8), nn_sfc%moisture_scale) / 1000.0

...
def draw_hf(df, show=False, save=True, save_path=None):
    ...
    yStr.append(mo_version + ex_pred2)
    yStr.append('MOST_chopped_' + ex_pred2)
    yStr.append('MOST_rounded_' + ex_pred2)
    #yStr.append(predictand2+ mo_version2)

    df2 = df.loc[df[mo_version + ex_pred2].isna() == False]
...
def drawTimeSeries(ax, predictions, exact_predictand, predictand, model_type="neural_network", colorPred='orange'):
    ...
    if model_type.startswith('MOST_'):
        y_pred = model_type + exact_predictand
...
if __name__ == "__main__":
    ...
    pred2 = 'heat_flux'
    ex_pred2 = 'heat_flux:18.4_m:degrees_K_m_s-1' # degrees celsius , meters per second

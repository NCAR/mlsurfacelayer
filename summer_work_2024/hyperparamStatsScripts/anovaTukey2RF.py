...
params = ['n_estimators', 'max_features', 'max_leaf_nodes']

# Extract hyperparameters and add them as separate columns
combined_df[params] = combined_df.apply(
...
grouped = combined_df.groupby(params).agg(
...
for _, group in filtered_grouped.iterrows():
    scores = combined_df[
        (combined_df['n_estimators'] == group['n_estimators']) &
        (combined_df['max_features'] == group['max_features']) &
        (combined_df['max_leaf_nodes'] == group['max_leaf_nodes']) 
    ]['Score']
...
if len(anova_data) > 1:
    ...
    if anova_result.pvalue < 0.05:
        ...
        combined_df['hyperparameter_combination'] = combined_df.apply(
            lambda row: (row['n_estimators'], row['max_features'], row['max_leaf_nodes']), axis=1
        )
        ...
        best_set = tuple(best_hyperparameters[['n_estimators', 'max_features', 'max_leaf_nodes']])
        print(best_set)
